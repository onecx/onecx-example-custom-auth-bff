...
import gen.org.tkit.onecx.extensions.bff.rs.internal.model.ProblemDetailInvalidParamDTO;
import gen.org.tkit.onecx.extensions.bff.rs.internal.model.ProblemDetailParamDTO;
import gen.org.tkit.onecx.extensions.bff.rs.internal.model.ProblemDetailResponseDTO;

@Mapper(uses = { OffsetDateTimeMapper.class })
public interface ExceptionMapper {

    default Response acidBadRequest(String message) {
        var dto = exception(ErrorKeys.ACID.name(), message);
        return Response.status(Response.Status.BAD_REQUEST).entity(dto).build();
    }

    default Response badRequest(String message) {
        var dto = exception(ErrorKeys.CONSTRAINT_VIOLATIONS.name(), message);
        return Response.status(Response.Status.BAD_REQUEST).entity(dto).build();
    }
